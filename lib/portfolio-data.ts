...
type Project = { name: string; category: string; year: string; type: "live" | "web" | "api" | "security"; domain?: string; image?: string; desc: string; tags: string[]; role: string; scope: string[]; links: ProjectLink[] };
// Live products come first and render with a site screenshot; the rest use illustrated cards.
export const projects: Project[] = [
    {
        name: "Jeon.id", category: "Creator platform", year: "2026", type: "live", domain: "jeon.id", image: "/projects/jeon.jpg", desc: "An all-in-one link-in-bio platform for Indonesian creators to share content, sell digital products, and grow their audience from a single page.", tags: ["Next.js", "Golang", "PostgreSQL"], role: "Full-stack Developer", scope: ["Built the creator dashboard and public profile pages with Next.js.", "Developed the REST API in Go (Gin) on PostgreSQL and Redis.", "Supports custom themes, digital product sales, memberships, and click and revenue analytics.", "Containerized the stack with Docker and automated staging and production deploys with GitHub Actions."], links: [{ label: "Visit jeon.id", href: "https://jeon.id" }]
    },
    {
        name: "PIKO Queue System", category: "Queue management", year: "2025", type: "live", domain: "antrian.piko.co.id", image: "/projects/antrian.jpg", desc: "A car queue management system for PIKO Carwash & Cafe that registers incoming vehicles and keeps the wash queue organized for staff.", tags: ["Laravel", "PHP", "Queue system"], role: "Web Programmer", scope: ["Built the Laravel application behind the car wash queue, from vehicle registration to queue status.", "Restricted the queue form to signed-in staff accounts.", "Deployed and maintain it in production as part of PIKO's internal systems."], links: [{ label: "Visit antrian.piko.co.id", href: "https://antrian.piko.co.id" }]
    },
    {
        name: "PIKO Inventory System", category: "Inventory management", year: "2025", type: "live", domain: "d.piko.co.id", image: "/projects/inventaris.jpg", desc: "StokApp, an inventory system that tracks PIKO's stock and production, installable as a progressive web app for quick access on staff devices.", tags: ["Next.js", "PWA", "Inventory"], role: "Web Programmer", scope: ["Built the stock and production management app with Next.js.", "Made it installable as a PWA so staff can launch it from the home screen.", "Runs in production behind account sign-in for PIKO staff."], links: [{ label: "Visit d.piko.co.id", href: "https://d.piko.co.id" }]
    },
    {
        name: "Noodu", category: "Learning platform", year: "2026", type: "live", domain: "noodu.id", image: "/projects/noodu.jpg", desc: "A project-based learning platform for digital careers, where learners build real projects with practitioner mentors instead of only watching videos.", tags: ["Next.js", "PostgreSQL", "Redis"], role: "Web Developer", scope: ["Developed the Next.js platform, including the CMS-driven public website.", "Supports three learning paths: one-on-one case project mentoring, cohort training, and self-paced courses with quizzes and certificates.", "Structured as a Turborepo monorepo on PostgreSQL and Redis."], links: [{ label: "Visit noodu.id", href: "https://noodu.id" }]
    },
    {
